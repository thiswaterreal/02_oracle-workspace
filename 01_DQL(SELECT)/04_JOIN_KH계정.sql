...
-- 1. 부서가 인사관리부인 사원들의 사번, 이름, 보너스 조회    (컬럼명 달라)
-- >> 오라클 전용 구문
...
-- 3. 보너스를 받는 사원들의 사번, 사원명, 보너스, 부서명 조회 (컬럼명 달라)
-- >> 오라클 전용 구문
SELECT EMP_ID, EMP_NAME, BONUS, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- 부서 배치가 아직 안된 사원 2명에 대한 정보가 조회 X (EMPLOYEE 기준)
-- 부서에 배정된 사원이없는 부서 같은 경우도 조회 X (DEPARTMENT 기준)
...
-- 이제, 사수 없는 사람들까지 모두 나옴 (LEFT JOIN)
...
SELECT * FROM LOCATION;     --(             LOCAL_CODE,     NATIONAL_NAME)
...
--JOIN NATIONAL N ON (L.NATIONAL_CODE = N.NATIONAL_NAME);
...
-- 1. 사번, 사원명, 부서명, 지역명, 국가명 조회 (EMP, DEP, LOC, NAT 조인)
SELECT * FROM EMPLOYEE; -- DEPT_CODE
SELECT * FROM DEPARTMENT; -- DEPT_ID  LOCATION_ID(L1)
...
SELECT * FROM EMPLOYEE; -- DEPT_CODE
SELECT * FROM DEPARTMENT; -- DEPT_ID LOCATION_ID
SELECT * FROM JOB; -- JOB_CODE로 동일함
SELECT * FROM LOCATION; --            LOCAL_CODE
SELECT * FROM SAL_GRADE; -- SAL_LEVEL 로 동일함
