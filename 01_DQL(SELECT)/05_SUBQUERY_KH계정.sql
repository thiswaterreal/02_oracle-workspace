...
/*
    * 서브쿼리 (SUBQUERY) == '부조회'
    ...
    - SUBQUERY(서브쿼리)     는 주로 WHERE 조건절에서 활용!
    - IN LINE VIEW(인라인뷰) 는 주로 FROM 절에서 활용!
*/
...
/*
    * 서브쿼리의 구분
      ...
      - 다중행 다중열 서브쿼리 : 서브쿼리 조회 결과값이 여러행 여러컬럼일 때 (여러행 여러열)       / () IN (거의 GROUP BY 절 포함)
...
WHERE JOB_NAME = '대리'
AND SALARY > ANY (2200000, 2500000, 3760000);       -- 이 중 가장 작은 값보다 크기만 하면 됨! (ANY!!)
...
/*
    ...
    ( 따라서 FROM 절에 쓰임. 별칭 붙여주기 가능!)
    ( 따라서 WHERE 절에서 별칭 사용 가능!)
*/
