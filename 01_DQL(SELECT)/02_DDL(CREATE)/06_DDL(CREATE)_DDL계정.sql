...
-- 데이터 딕셔너리 : 다양한 객체들의 정보를 저장하고 있는 시스템 데이블들
...
-- INSERT INOT 테이블명 VALUES (값1, 값2, 값3 ...);
...
/*
    ...
    * NOT NULL 제약조건은 오로지 컬럼레발방식!! 밖에 안됨 **중요**
*/
...
--> UNIQUE 제약조건에 위배되어있음! INSERT 실패!
--> 오류구문을 제약조건명으로 알려줌! (특정 컬럼에 어떤 문제가 있는지 상세히 알려주지는 않음)
--> 쉽게 파악하기 어려움..
--> 제약조건 부여시, 제약조건명을 지정해주지 않으면 시스템에서 임의로 제약조건명을 부여해버림
...
INSERT INTO MEM_UNIQUE VALUES(1, 'user01', 'pass01', '손흥민', null, null,null);
INSERT INTO MEM_UNIQUE VALUES(2, 'user01', 'pass02', '이강인', null, null,null);
-- ORA-00001: unique constraint (DDL.MEMID_UQ) violated     -- 아,, MEMID_UQ 오류났구나

INSERT INTO MEM_UNIQUE VALUES(2, 'user02', 'pass02', '이강인', null, null,null);
INSERT INTO MEM_UNIQUE VALUES(3, 'user03', 'pass02', '이강인', 'ㄴ', null,null);
--> 성별에 유효한 값이 아니어도 잘 INSERT가 된다 => 이러면 안됨..
...
INSERT INTO MEM_CHECK VALUES(2, 'user02', 'pass02', '이강인', 'ㄴ', null, null);
-- 만약 GENDER 컬럼에 데이터 값을 넣고자 한다면 CHECK 제약조건에 만족하는 값을 넣어줘야됨
-- NOT NULL 아니면 NULL 도 가능하긴 함!!
-- CHECK 제약조건 == 입구컷!
...
/*
    * PRIMARY KEY(기본키) 제약조건 => PK
    테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약조건 (마치 식별자의 역할)
...
CREATE TABLE MEM_PRI(
    MEM_NO NUMBER CONSTRAINT MEMNO_PK PRIMARY KEY,     -- 메인 식별 너로 정했다!
    MEM_ID VARCHAR2(20) NOT NULL UNIQUE,
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CHECK(GENDER IN ('남', '여')),  -- 컬럼레벨방식
...
-- PRIMARY KEY 로 묶여있는 각 컬럼에는 절대 NULL을 허용하지 않음

---------------------------------------------------------------------------------
-- 복합키 사용 예시 (찜하기, 좋아요, 구독)
-- 찜하기 : 한 상품을 오로지 한번만 찜할 수 있음
...
INSERT INTO TB_LIKE VALUES(1, '감자', SYSDATE);   -- 에러 발생!! 찜은 한번만!!
...
-- 40은 유효한 회원등급 번호가 아님에도 불구하고 잘 insert 됨..
...
-- 10, 20, 30, NULL 까지도 들어는 감.. 다른값은 불가능.
