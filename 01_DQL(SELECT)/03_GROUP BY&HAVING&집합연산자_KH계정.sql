...
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
WHERE AVG(SALARY) >= 3000000    -- 오류!! (GROUP BY 이후에 그 분류별 그룹함수() 적용하여 값을 조회하기 때문에.. WHERE -> GROUP BY -> HAVING -> SELECT 순서)
GROUP BY DEPT_CODE;
...
FROM EMPLOYEE
WHERE SALARY > 3000000;
-- 컬럼 개수 뿐만 아니라 각 컬럼 자리마다 동일한 타입으로 기술해야함 (SALARY, BONUS 둘 다 NUMBER타입)
