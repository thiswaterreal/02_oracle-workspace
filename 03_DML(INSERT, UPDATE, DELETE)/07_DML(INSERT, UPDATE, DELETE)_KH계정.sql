...
SELECT EMP_NAME, SALARY, BONUS      -- 방명수 / 1518000	/ NULL
FROM EMP_SALARY
WHERE EMP_NAME = '방명수';

SELECT EMP_NAME, SALARY, BONUS      -- 유재식 / 3740000	/ 0.2
FROM EMP_SALARY
WHERE EMP_NAME = '유재식';

-- 단일행 서브쿼리 (1행1열)
UPDATE EMP_SALARY
SET SALARY = (SELECT SALARY FROM EMP_SALARY WHERE EMP_NAME = '유재식'),    -- 3740000
    BONUS = (SELECT BONUS FROM EMP_SALARY WHERE EMP_NAME = '유재식')       -- 0.2
